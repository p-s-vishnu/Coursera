...
%         computed in ex4.m
    ...
    labelMatrix = zeros(m , num_labels);
    for i= 1:m
       labelMatrix( i, y(i) ) = 1;
    end
    y = labelMatrix;
    ...
    RegTerm = lambda/(2*m) * ( sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2,2)) );
    J   = J + RegTerm;
...
for i=1:m
    z2 = Theta1 * a1(i,:)';     % 25x401x1
    a2 = sigmoid(z2);          % 25x1
    
    z3 = Theta2 * [1 ; a2]; % 10x26x1
    a3 = sigmoid(z3);       % 10x1
    
    delta_3 = a3 - y(i,:)'; % 10x1

    delta_2 = (Theta2' * delta_3) .* sigmoidGradient([1; z2]); % 26x10x1
    
    delta_2 = delta_2(2:end); % 25x1

    Theta2_grad = Theta2_grad + delta_3 * [1 ; a2]'; % 10x1x26
    Theta1_grad = Theta1_grad + delta_2 * a1(i,:); % 25x1x401  
end
